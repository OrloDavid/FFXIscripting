...
Func _FileListToArrayEx($sPath, $sFilter = "*", $iFlag = 0)
    ...
    If Not BitAND($iFlag, 64) And StringRegExp($sFilter, "^\s*$|[\\/:><]|\v") Then Return SetError(2, 2, "")
    Local $hSearch, $sFile, $sFileList, $iFlags = StringReplace(BitAND($iFlag, 1) + BitAND($iFlag, 2), "3", "0"), $sSubDir = BitAND($iFlag, 4), $DirPath = StringRegExpReplace(StringReplace(BitAND($iFlag, 8), "8", $sPath), "^0", "")
    $hSearch = FileFindFirstFile($sPath & "*")
    If @Error Then Return SetError(3, 3, "")
	Local $hWSearch = $hSearch, $hWSTMP = $hSearch, $SearchWD, $Extended, $sDirFilter = StringReplace($sFilter, "*", ""), $sDelim = StringRegExpReplace(StringReplace("|" & BitAND($iFlag, 128) & BitAND($iFlag, 8), "128", "\"), "\\8|\d+", "")
